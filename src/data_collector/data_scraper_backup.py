...
def handle_sigint(signal_received, frame):
    """
    Handle SIGINT and SIGTERM signals for graceful shutdown.def handle_sigint(signal_received, frame):
    """
...
def save_data_to_parquet(df, symbol):
    ...
    try:
        ...
        combined_df = combined_df.drop_duplicates(subset=["timestamp"]).sort_values("timestamp")        # Save the deduplicated DataFrame locally using our new local storage
...
def signal_handler(sig, frame, twm=None):
    """
    Handle SIGINT and SIGTERM signals for graceful shutdown.
    """
...
# Note: check_and_pause_if_bucket_full is now handled by local_storage module
